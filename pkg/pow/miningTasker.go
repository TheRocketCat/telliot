...
func (mt *MiningTasker) GetWork(chan *Work) (*Work, bool) {
	...
	if err != nil {
		level.Error(mt.logger).Log("msg", "could not get data from data proxy, cannot continue at all")
		return nil, false
	}
	...
	level.Debug(mt.logger).Log("msg", "this long since last value", "time", today.Sub(tm))
	...
	for i := 0; i < 5; i++ {
		...
		if err != nil {
			level.Info(mt.logger).Log("msg", "could not retrieve pricing data for current request id", "err", err)
			//return nil, false
		}
		val := m2[valKey]
		if len(val) == 0 {
